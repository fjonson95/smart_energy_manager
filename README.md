...
![Version](https://img.shields.io/badge/version-0.9.57-blue)
...
The default Legionella run duration is now 110 minutes (was 60), matching the boiler program's measured runtime. An already-saved value is kept. See [CHANGELOG.md](CHANGELOG.md) for the full writeup.

See [CHANGELOG.md](CHANGELOG.md) for older releases (including v0.9.56's Legionella reason attribute, v0.9.55's flexible 5–9 day Legionella interval with cheapest-window choice, v0.9.54's buy-price and V lines in the plan card, v0.9.53's fix for the load-shape percentile producing a physically impossible daily total, v0.9.52's exposure of the planner's marginal-value bookkeeping, v0.9.51's fix for the planner projecting near-zero household load on a mild day, v0.9.50's plan-card color pass for light/dark themes, v0.9.49's fix for the executor silently refusing to charge on solar surplus, v0.9.48's Solcast-based reserve before tomorrow's prices are published, v0.9.47's export-figure fix in the plan card, v0.9.46's fix for the planner reserving battery capacity for the day's most expensive slot at the expense of an earlier, cheaper-but-still-valuable one, v0.9.45's battery-cost reset at low SOC, v0.9.44's hold-battery fix for "idle" plan slots — winter backtest savings more than doubled, 93 → 217 SEK, v0.9.43's decision-sensor de-noising, v0.9.42's decision-text rewrite, v0.9.41's merit-order earmark fix for self-consumption, v0.9.40's cheapest-first grid charging, v0.9.39's plan-card color fix, v0.9.38's shape-aware evening-fill wiring, v0.9.37's hourly-shape data-loss fix, v0.9.36's phase-limit double-call fix, v0.9.35's hot-water-dump surplus-ratio fix, v0.9.34's battery phase-limit oscillation fix, v0.9.33's charger card battery bar fix, v0.9.32's EV deadline scheduling, v0.9.31's hot-water-dump wiring, v0.9.30's boiler solar-mode wiring, v0.9.29's weekly export-margin tracking, v0.9.28's auxiliary heater baseline tracking, v0.9.27's drought-risk markup wiring, v0.9.26's real-current phase-limit fix, v0.9.25's step 7 points 2 & 5, v0.9.24's solar_charge self-consumption fallback fix, v0.9.23's export-history service, v0.9.22's dead-config cleanup, v0.9.21's damped-temp crash fix, v0.9.20's Solcast entity split, v0.9.19's rule 3/4 grid-charge/export fix, v0.9.7's reserve trajectory floor, v0.9.6's shape×level load model, v0.9.5's reworked dump-energy detection, v0.9.4's equivalent-cycles sensor, v0.9.3's measured round-trip efficiency, and v0.9.2's executor-veto fix).
